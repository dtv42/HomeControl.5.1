namespace ModbusTest
{
    ...
    public class TestModbusRTU : IClassFixture<WebApplicationFactory<ModbusRTU.Startup>>
    {
        private readonly WebApplicationFactory<ModbusRTU.Startup> _factory;
        private readonly JsonSerializerOptions _options = new JsonSerializerOptions();

        public TestModbusRTU(WebApplicationFactory<ModbusRTU.Startup> factory)
        {
            _factory = factory;
        ...
        public async Task TestSettingsController()
        {
            ...
            Assert.Equal("application/json; charset=utf-8", response.Content.Headers.ContentType.ToString());
            var json = await response.Content.ReadAsStringAsync();
            var settings = JsonSerializer.Deserialize<RtuClientSettings>(json, _options);

            Assert.Equal("COM1", settings.RtuMaster.SerialPort);
            Assert.Equal(19200, settings.RtuMaster.Baudrate);
            Assert.Equal(10000, settings.RtuMaster.ReadTimeout);
            Assert.Equal(10000, settings.RtuMaster.WriteTimeout);
            Assert.Equal(1, (int)settings.RtuSlave.ID);
        }
    }
}
